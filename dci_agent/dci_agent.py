...
"""
    ...
    with open(mirror_location + '/' + ctx.last_job_id + '.repo', 'w') as f:
        for c in components:
            ...
            component_file = dci_component.list_files(ctx, c['id']).json()['component_files'][0]
            # TODO(Goneri): test the md5/size of the existing tarball first
            if not os.path.exists(tarball_dest):
                dci_component.download_file(ctx, c['id'], component_file['id'], tarball_dest)
            ...
            f.write(repo_entry.format(
                ...
                path='/' + ctx.last_job_id + '/' + c['canonical_project_name']))
